...
class OciClientFactory:
    ...
    def signer_and_config(self) -> tuple[Any | None, dict[str, Any]]:
        oci = self._load_oci()

        if self.settings.oci_auth == "instance_principal":
            signer = oci.auth.signers.InstancePrincipalsSecurityTokenSigner()
            return signer, {"region": self.settings.home_region}
        ...
        if self.settings.home_region:
            config["region"] = self.settings.home_region
        return None, config
