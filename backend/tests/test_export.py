...
def test_export_completeness_csv_returns_csv():
    from app.main import create_app
    client = TestClient(create_app())
    response = client.get("/api/export/completeness.csv?regions=eu-frankfurt-1")
    assert response.status_code == 200
    assert "text/csv" in response.headers["content-type"]
    lines = response.text.splitlines()
    assert lines[0].startswith("region,subscribed,requested,status")
    assert "eu-frankfurt-1" in lines[1]
